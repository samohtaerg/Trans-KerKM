# -*- coding: utf-8 -*-
"""HPC Kernel KM (Feature)

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1bIwsxBWR0kPcTdxNQCJzr4Px1ExdlyUV
"""
...
def kernel_weighted_transfer_km(X_source, Y_source, delta_source,
    ...
    Args:
        ...
        Y_source: survival times of source data
        delta_source: event indicators of source data
        X_train_target: feature matrix of target train data
        Y_train_target: survival times of target train data
        delta_train_target: event indicators of target train data
        X_test_target: feature matrix of target test data
        Y_test_target: survival times of target test data
        delta_test_target: event indicators of target test data
...
def fit_transfer_cox_model(X_source, Y_source, delta_source,
                          X_train_target, Y_train_target, delta_train_target,
                          X_test_target, Y_test_target, delta_test_target,
                          col_names=None, random_state=None):
   """
...
def generate_multigroup_data(n_source=1000, n_target=200, n_groups=5, source_mix=None, target_mix=None, distribution_type=None, data_type=None,
    ...
    # signal_strength = "strong" #@param ["strong", "weak"]
...
def run_experiment_with_models(X_source, Y_source, delta_source,
    ...
    return {
        ...
        'data': {
            ...
            'group_labels_train_target': group_labels_train_target, 'group_labels_test_target': group_labels_test_target
        }
    }
...
if __name__ == "__main__":
    ...
    if param_grid_type == "comprehensive":
        ...
        sigma_grid = np.logspace(np.log10(1), np.log10(10.0), 5).tolist()
        lambda_grid = np.logspace(np.log10(0.1), np.log10(5.0), 5).tolist()
    ...
    # Dictionary to store results for all feature spread values
    all_results = {}

    # Define feature spread values to explore
    feature_spread_values = [0, 0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4]

    # Run experiments for all feature spread values
    for feature_spread in feature_spread_values:
        print(f"\n{'='*20} FEATURE SPREAD {feature_spread} {'='*20}")

        # Generate experimental data
        print(f"Generating multi-group simulated data with feature spread {feature_spread}...")
        ...
        group_labels_source, _ = generate_multigroup_data(
            ...
            feature_spread=feature_spread,  # Pass the feature spread parameter
        ...
        # Add simplified results for this feature spread
        spread_results = {}
        for key in ['Cox (Target Only)', 'Cox (Naive Transfer)', 'Feature-based KM', 'Kernel-weighted KM (Target Only)', 'Prediction-based KM']:
            if key in results:
                ...
                spread_results[key] = float(results[key])

        # Store in all_results dictionary
        all_results[f"feature_spread_{feature_spread}"] = spread_results
