...
def generate_multigroup_data(n_source=1000, n_target=200, n_groups=5, source_mix=None, target_mix=None, distribution_type=None, data_type=None, signal_level=None, seed=111):
    ...
    if distribution_type == "separate":
        group_distributions = [
            {'mean': [35, 1.5], 'std': [3, 0.5]},
            {'mean': [50, 2.0], 'std': [3, 0.5]},
            {'mean': [65, 3.0], 'std': [3, 0.5]},
            {'mean': [30, 5.0], 'std': [3, 0.5]},
            {'mean': [80, 5.5], 'std': [3, 0.5]}
        ]
...
#             print(f"  Group {g+1}: beta = {group_betas[g]}")
    ...
    for g in range(n_groups):
        ...
        C = np.random.exponential(scale=1000, size=n_g)
        ...
        n_events = np.sum(delta)
        # Calculate how many we want to censor
        target_censoring = min(n_events - 1, int(n_g * group_censoring_rates[g]))

        if target_censoring > 0 and n_events > 0:
            # Apply some censoring manually to match desired rate
            event_indices = np.where(delta == 1)[0]
            # Make sure we don't try to censor more than we have
            n_to_censor = min(len(event_indices), target_censoring)
            if n_to_censor > 0:
                censor_indices = np.random.choice(
                    event_indices,
                    size=n_to_censor,
                    replace=False
                )
                delta[censor_indices] = 0
